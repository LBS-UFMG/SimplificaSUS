...
print(len(dic_qualis), len(dic_issn))
print("PARTE 3: vinculando FATOR DE IMPACTO + QUALIS")
print("ISSN;PERIÓDICO;QUALIS;FI (JCR);H-index;Citações (~3y);Citações (média:~3y)")
unico = []
...
for i in dic_qualis:
	...
	nome = dic_qualis[i][1]
	nome = nome.replace(";","-")
	qualis = dic_qualis[i][2]

	try:
		print(issn, dic_issn[i][1].replace(";","-"), qualis, round(float(dic_issn[i][2]), 2), int(dic_issn[i][3]), int(dic_issn[i][4]), round(float(dic_issn[i][6])), sep=";")
	except:
		if issn not in unico:
			print(issn, nome, qualis, '0', '0', '0', '0',sep=";")
		else:
			erro = 1
