...
'''
key = 'e95d00dd2eec638f71b80c430978c1f6'
nova_key = 'a07b4941e3d35b5c0fe65066c74e4b38'

headers = {'X-ELS-APIKey': 'e95d00dd2eec638f71b80c430978c1f6'}

i = 0
w = open("saida_"+str(rg)+".csv", 'w')
with open('doi.csv', newline='') as csvfile:
  spamreader = csv.reader(csvfile, delimiter=';', quotechar='|')
  for row in spamreader:
    i+=1
    if i >= rg and i < rg+1000:
      # id = row[0]
      # cit = row[1]
      doi = row[0]
      while True:
        url = "https://api.elsevier.com/content/search/scopus?query=DOI("+doi+")&field=citedby-count"
        ...
        print(string)
        exit()
        x = re.findall('"citedby-count":"[0-9]*"', string)
        y = int(x[0].replace('"citedby-count":"','').replace('"',''))
        #print(cit, y)
        w.write(id+";"+str(y)+"\n")
        #i+=1
        #print(i)
